...
/**
 * Пользовательская реализация изменяемого списка. Этот класс реализует интерфейс {@link IntensiveList}
 * и предоставляет базовые операции, такие как добавление, удаление и получение
 * элементов, а также сортировка и разделение списка.
 *
...
public class ArrayList_MihailKrasheninnikov<E> implements IntensiveList<E> {
  ...
  /**
   * Вставляет указанный элемент в указанную позицию списка. Сдвигает элемент, который находится на
   * этой позиции (если есть), и все последующие элементы на одну позицию вправо.
   *
  ...
  /**
   * Сортирует список с использованием алгоритма быстрой сортировки.Дефолиное решени, решение из
   * книги со стримами требует заимплиментить List<E>
   *
